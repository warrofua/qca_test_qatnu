    ...
    def get_bond_dimension(self, bond_index, state):
        ...
        for idx in range(self.total_dim):
            ...
            if prob > 0:
                ...
                norm += prob
        return 2 ** (avgN / norm) if norm > 0 else 1
...
def fit_ramsey_frequency(data):
    """Fit Ramsey oscillation frequency using FFT (robust and fast)"""
    ...
    frequency = 2 * np.pi * freqs[pos_mask][peak_idx]
    
    # Clamp to reasonable range
    dt = times[1] - times[0]
    frequency = np.clip(frequency, 0.1, 5.0)
