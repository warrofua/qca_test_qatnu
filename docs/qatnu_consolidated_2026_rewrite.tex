...
\hypersetup{
  ...
  pdftitle={SRQID/QATNU as a Unitary Lattice Testbed for Scalar Emergent Geometry},
...
\title{\textbf{SRQID/QATNU as a Unitary Lattice Testbed for Scalar Emergent Geometry:}\\
\textbf{Closed Scalar Observable Chain, Topology Dependence, and Tensor Falsification}}
...
\section{Canonical model and closed scalar observable chain}
...
\subsection{Closed scalar observable chain}

The cleanest theorem candidate currently supported by the repository is not a tensor statement. It is the following scalar chain:
\begin{equation}
\text{local lattice dynamics}
\rightarrow
\text{source-induced bond occupation}
\rightarrow
\text{sitewise scalar load }\Lambda_i
\rightarrow
\text{clock/redshift observables}.
\label{eq:scalar-chain}
\end{equation}

Concretely, define the bond occupation operator by
\begin{equation}
n_{ij} = \sum_m m\,\Pi_{ij}^{(m)},
\label{eq:bond-occupation}
\end{equation}
and define the sitewise scalar load by the operational dilute-regime expression
\begin{equation}
\Lambda_i
\equiv
\sum_{j\in \partial i}\log\!\bigl(1+\langle n_{ij}\rangle\bigr).
\label{eq:lambda-operational}
\end{equation}
Then the primary observable family closes immediately:
\begin{equation}
\omega_{\mathrm{eff}}^{(i)}
=
\frac{\omega}{1+\alpha \Lambda_i},
\qquad
\frac{\omega_{\mathrm{eff}}^{(a)}}{\omega_{\mathrm{eff}}^{(b)}}
=
\frac{1+\alpha\Lambda_b}{1+\alpha\Lambda_a},
\qquad
z_{a\leftarrow b}
=
\frac{1+\alpha\Lambda_a}{1+\alpha\Lambda_b}-1.
\label{eq:redshift-family}
\end{equation}

The scientific content of Eq.~\eqref{eq:redshift-family} is straightforward: a localized promoted source changes edge occupations; those occupations define a sitewise scalar load $\Lambda_i$; and that load renormalizes local clock rates and the associated lattice redshift ratios. This is the strongest end-to-end chain currently supported by the code, decision logs, and reproducible figures.

At this stage it is important to state what Eq.~\eqref{eq:redshift-family} does \emph{not} claim. It is not yet a continuum theorem, not yet a tensor derivation, and not yet a proof that the scalar load composes into full gravity at large scale. It is a closed \emph{lattice-internal observable chain}, and that is exactly why it should now be the center of the paper.

\subsection{Derived-\texorpdfstring{$\alpha$}{alpha} route versus phenomenological-\texorpdfstring{$\alpha$}{alpha} scans}

The perturbative note in the repository gives the cleanest currently available route for interpreting $\alpha$ as a derived scalar susceptibility rather than only a fit knob. In the dilute weak-coupling regime it yields the schematic scaling
\begin{equation}
\alpha_i
\approx
\frac{2\lambda^2}{\omega^2\Delta_{\mathrm{eff}}}\,\bar F_i,
\label{eq:alpha-pert}
\end{equation}
with $\bar F_i$ the local frustration-weighted occupation average. This is conceptually important because it ties the scalar constitutive law to the microscopic promotion mechanism.

However, Eq.~\eqref{eq:alpha-pert} should still be treated as a \emph{supporting derivation route}, not as a numerically closed replacement for the phenomenological $\alpha$ used in the main scans. The current production workflow still studies scan values such as $\alpha=0.6,0.8,1.0$, and the bridge between those phenomenological values and the microscopic susceptibility extraction is not yet fully closed.
...
\begin{quote}
\emph{SRQID/QATNU is a strictly unitary, measurement-free, bounded-degree quantum lattice model in which source-induced bond promotion generates an operational scalar load $\Lambda$ that renormalizes local clock rates and defines a family of topology-dependent lattice redshift observables.}
\end{quote}
...
\subsection{Lattice shell averages and optional effective closure}

The repo now has a reasonably solid \emph{scalar} story but still lacks a clean continuum closure that turns shell-averaged promotions into a nonsingular radial profile. The safest radial presentation is therefore still lattice-internal:
\begin{equation}
\Lambda_{\mathrm{shell}}(r)
=
\frac{1}{|S_r|}\sum_{i\in S_r}\Lambda_i,
\qquad
\omega_{\mathrm{eff}}(r)
=
\frac{\omega}{1+\alpha \Lambda_{\mathrm{shell}}(r)}.
\label{eq:shell-average}
\end{equation}
Equation~\eqref{eq:shell-average} keeps the scalar theorem entirely inside the measured lattice observables. It is the right main-text closure because it does not assume a continuum ansatz that the simulator has not yet derived.

If one wants an \emph{effective} continuum bridge beyond the lattice theorem, the most conservative ansatz consistent with the current scalar narrative is to introduce a coarse-grained radial field
\begin{equation}
\Lambda_{\mathrm{cg}}(r)
=
\Lambda_0\,j_0(kr)
...
\begin{enumerate}
    \item Eq.~\eqref{eq:shell-average} is the stronger present statement because it stays inside measured lattice data.
    ...
    \item It should be used as an optional effective background around which any future tensor sector is defined, not as a substitute for that tensor sector.
\end{enumerate}
...
\begin{quote}
\emph{a disciplined unitary lattice testbed has been built, its scalar observable chain now closes cleanly enough to stand as the core result, its topology dependence is real, and its tensor sector has now been constrained tightly enough that failure is scientifically informative rather than merely disappointing.}
\end{quote}
